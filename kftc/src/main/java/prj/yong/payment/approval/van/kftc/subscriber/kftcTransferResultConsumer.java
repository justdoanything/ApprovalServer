...
public class KftcTransferResultConsumer {
    ...
    @KafkaListener(topics = "${kftc.transfer.cancel.fdk.topic.name}", containerFactory = "kftcTransferResultKafkaListenerContainerFactory")
    private void kftcTransferCancelFdkListener(TransactionHistory transactionHistory, Acknowledgment ack) throws Exception {
        LOGGER.info("Received KFTC-To-Fdk Mang Cancel result message : ");

        try {
            // 보상 트랜잭션에 대한 처리 시작
    
            // 망취소 호출
            fdkFeignClient.mangCancel(transactionHistory);
